...
export default function Testimonials() {
  ...
  // Hàm format ngày - cũng nên dịch luôn cho chuẩn
  const formatDate = (dateString) => {
    if (!dateString) return t('testimonials.noDate'); // "Không rõ"
    const date = new Date(dateString);
    const now = new Date();
    const diffInDays = Math.floor((now - date) / (1000 * 60 * 60 * 24));

    if (diffInDays === 0) return t('testimonials.today'); // "Hôm nay"
    if (diffInDays === 1) return t('testimonials.yesterday'); // "Hôm qua"
    if (diffInDays < 7) return t('testimonials.daysAgo', { count: diffInDays }); // "X ngày trước"
  ...
  const fetchSummary = async () => {
    try {
      ...
      if (!res.ok) throw new Error(t('testimonials.errorLoad')); // "Không thể tải đánh giá"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mt-2 mb-3">
            {t('testimonials.title')} {/* Bệnh Nhân Nói Gì Về Chúng Tôi */}
          </h2>
        </div>
        ...
        <div className="flex justify-center mb-12">
          <div className="inline-flex items-center bg-white rounded-2xl shadow-lg border border-cyan-100 p-6 hover:shadow-xl transition-shadow">
            ...
            <div className="text-center ml-6">
              ...
              <div className="text-sm text-gray-600">{t('testimonials.recommendation')}</div>
            </div>
          </div>
        </div>
        ...
        {reviews.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500">{t('testimonials.noReviews')}</p>
          </div>
        ) : (
              ...
              >
                <div className="flex-1 flex flex-col">
                  ...
                  <div className="flex items-center gap-3 text-base mt-2">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900 group-hover:text-cyan-600 transition-colors duration-300">
                        {review.reviewerName}
                      </p>
