...
const StatisticsPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // FIX: Use authService to get token (key is 'auth_token', not 'token')
                const token = authService.getToken();

                console.log('Statistics.jsx: Fetching data...', {
                    startDate,
                    endDate,
                    statType,
            ...
            <header className={`flex flex-wrap items-center justify-between gap-4 rounded-2xl border ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'} ${isMobile ? 'p-3' : 'p-4'} shadow-sm transition-colors duration-300`}>
                ...
                <div className={`flex ${isMobile ? 'flex-col w-full' : 'flex-wrap'} items-center gap-4`}>
                    <div className={`flex ${isMobile ? 'w-full overflow-x-auto' : ''} rounded-full p-1 ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-100'}`}>
                        {RANGE_OPTIONS.map((option) => (
                            <button
                                ...
                                className={`${isSmallMobile ? 'px-2 py-1 text-xs' : 'px-4 py-1.5 text-sm'} font-semibold rounded-full transition-all whitespace-nowrap ${option.value === rangeType
                    ...
                    <button
                        ...
                        disabled={exporting}
                        className={`flex items-center gap-2 ${isMobile ? 'w-full justify-center' : ''} ${isSmallMobile ? 'px-3 py-1.5 text-sm' : 'px-4 py-2'} rounded-lg font-semibold text-white transition-all shadow-sm ${exporting
