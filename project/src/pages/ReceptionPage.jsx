...
export default function ReceptionPage() {
  ...
  const handleLogout = async () => {
    try { await axiosInstance.post('/api/auth/logout'); } catch (error) { console.error(error); }
    finally { authService.logout(); navigate('/login', { replace: true }); }
  };
  ...
  const handleClearFilters = () => { setSearchPhone(''); setFilterStatus(''); loadQueueList(); };

  // -------- Thêm/Sửa Form --------
  const handleFormChange = (field, value) => { setPatientForm(prev => ({ ...prev, [field]: value })); };
  const handleAddPatient = () => { setPatientForm(emptyPatientForm); setEditPatientId(null); setShowForm(true); };
  ...
  const handleEditPatient = async (patient) => {
    try {
      ...
      setShowForm(true);
    } catch (err) { toast.error("Không tải được thông tin chi tiết!"); }
  };
  ...
  const handleDeletePatient = async (patientId) => {
    ...
    } catch { toast.error("Không thể xoá bệnh nhân!"); }
  };
  ...
  const handleSubmitForm = async () => {
    try {
      if (editPatientId) {
        ...
        setQueueList(prev => prev.map(p => p.queueId === editPatientId ? res.data : p));
        toast.success("Cập nhật bệnh nhân thành công!");
      } else {
        const res = await axiosInstance.post('/api/reception/queue/add', patientForm);
        setQueueList(prev => [res.data, ...prev]);
  ...
  const handleQuickUpdateStatus = async (queueId, status) => {
    try {
      ...
      setQueueList(prev => prev.map(p => p.queueId === queueId ? { ...p, status } : p));
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between space-x-4">
        <div className="flex space-x-2">
          ...
          <button className="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300" onClick={handleSearchQueue}>Tìm kiếm</button>
          <button className="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300" onClick={handleClearFilters}>Xóa lọc</button>
        </div>
      ...
      {showForm && (
        <div className="bg-white p-4 border rounded shadow space-y-2">
          <h3 className="font-semibold">{editPatientId ? 'Cập nhật bệnh nhân' : 'Thêm bệnh nhân'}</h3>
          <div className="grid grid-cols-2 gap-2">
            <input placeholder="Mã hàng đợi" value={patientForm.queueNumber} onChange={(e) => handleFormChange('queueNumber', e.target.value)} className="border px-2 py-1 rounded"/>
            ...
            <input type="datetime-local" value={patientForm.checkInTime} onChange={(e) => handleFormChange('checkInTime', e.target.value)} className="border px-2 py-1 rounded"/>
          </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Mã hàng đợi</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {queueList.map((q, index) => (
              <tr key={q.queueId} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">{priorityLabels[q.priority] || q.priority}</td>
                <td className="px-4 py-3">
                  <select value={q.status} onChange={(e) => handleQuickUpdateStatus(q.queueId, e.target.value)} className="border border-gray-300 rounded px-2 py-1 text-sm">
                    {Object.entries(statusLabels).map(([key, label]) => (<option key={key} value={key}>{label}</option>))}
                  </select>
                </td>
                <td className="px-4 py-3">{formatDateTime(q.checkInTime)}</td>
                <td className="px-4 py-3 space-x-1">
                  <button className="px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 text-xs" onClick={() => handleEditPatient(q)}>Sửa</button>
                  <button className="px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600 text-xs" onClick={() => handleDeletePatient(q.queueId)}>Xóa</button>
                </td>
              </tr>
            ))}
          </tbody>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 p-8 space-y-4">{activeMenu === 'records' && renderQueueSection()}</main>
      </div>
    </div>
  );
}
